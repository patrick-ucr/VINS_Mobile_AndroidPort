...
bool LoopClosure::startRemoteLocalize(std::vector<cv::KeyPoint> &keys, std::vector<BRIEF::bitset> &descriptors,
                                   std::vector<cv::Point2f> &cur_pts,
                                   std::vector<cv::Point2f> &old_pts,
                                   int &old_index){
    printf("LoopClosure::startRemoteLocalize");
    try
    {
        bool loop_succ = false;
        loop_succ = demo.runRemote("BRIEF", keys, descriptors, cur_pts, old_pts, old_index);
        if (loop_succ){
            printf("LoopClosure::startRemoteLocalize localize SUCCESS");
        } else {
            printf("LoopClosure::startRemoteLocalize localize FAIL");
        }
        return loop_succ;
    }
...
bool LoopClosure::startLoopClosure(std::vector<cv::KeyPoint> &keys, std::vector<BRIEF::bitset> &descriptors,
                                   ...
                                   int &old_index)
{
  printf("LoopClosure::startLoopClosure");
    try
  {
    bool loop_succ = false;
    loop_succ = demo.run("BRIEF", keys, descriptors, cur_pts, old_pts, old_index);
      if (loop_succ){
          printf("LoopClosure::startLoopClosure startLoopClosure found loop SUCCESS");
      } else {
          printf("LoopClosure::startLoopClosure startLoopClosure found loop FAIL");
      }
    return loop_succ;
  }
  catch(const std::string &ex)
