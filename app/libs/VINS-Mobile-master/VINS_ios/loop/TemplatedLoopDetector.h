...
#define LOG_TAG "TemplatedLoopDetector.h"
...
public:
  ...
  /* use keypoints from remote frames to localize in loop detection
   * localization only and avoid updating local variable e.g. m_database
   * relax some contraints in the four stages of loop detection
   * */
    bool localizeRemote(const std::vector<cv::KeyPoint> &keys,
                    const std::vector<TDescriptor> &descriptors,
                    DetectionResult &match,std::vector<cv::Point2f> &cur_pts,
                    std::vector<cv::Point2f> &old_pts);
  /**
...
// --------------------------------------------------------------------------
    template<class TDescriptor, class F>
    bool TemplatedLoopDetector<TDescriptor, F>::localizeRemote(
            const std::vector<cv::KeyPoint> &keys,
            const std::vector<TDescriptor> &descriptors,
            ...
            std::vector<cv::Point2f> &old_pts)
    {
        LOGI("TemplatedLoopDetector::localizeRemote()");

        //LOGI("TemplatedLoopDetector:detectLoop() m_params.k %d m_params.alpha %.2f"
        //      ,m_params.k, m_params.alpha);
        EntryId entry_id = m_database->size();
        LOGI("TemplatedLoopDetector::localizeRemote() entry_id %d", int(entry_id));
        ...
        if(m_params.geom_check == GEOM_DI) {
            //LOGI("TemplatedLoopDetector:detectLoop() GEOM_DI");
            m_database->getVocabulary()->transform(descriptors, bowvec, featvec,
                                                   m_params.di_levels);
        }
        else
            m_database->getVocabulary()->transform(descriptors, bowvec);
        LOGI("TemplatedLoopDetector:localizeRemote() entry_id %d m_params.dislocal %d",
             int(entry_id), int(m_params.dislocal));
        LOGI("TemplatedLoopDetector:localizeRemote() bowvec.size() %d featvec.size() %d",
             bowvec.size(), featvec.size());

        /* no need to limit this for remote localization
        ...
        {
            //printf("entry_id %d > m_params.dislocal %d\n",entry_id,m_params.dislocal);
            // temporal distance is at least dislocal
            // TODO adjust this for remote localization to improve recall
            //int max_id = (int)entry_id - m_params.dislocal;
            int max_id = (int)entry_id - 1;

            QueryResults qret;
            m_database->query(bowvec, qret, m_params.max_db_results, max_id);

            // no need to update database for remote localization
            //m_database->add(bowvec, featvec); // returns entry_id

            if(!qret.empty()) {
                LOGI("TemplatedLoopDetector:localizeRemote() QueryResults size: %d", qret.size());
                //printf("qret not empty\n");
                // factor to compute normalized similarity score, if necessary
                LOGI("TemplatedLoopDetector:localizeRemote() before removeLowScore best Id %d highest score %lf",
                        int(qret[0].Id), qret[0].Score);
                // TODO now use min_nss_factor as the threshold for an acceptable match
                if (qret[0].Score >= m_params.min_nss_factor){
                    match.match = qret[0].Id;
                    match.status = LOOP_DETECTED; // more like localization success
                ...
                if(m_params.use_nss)
                {
                    ...
                    LOGI("TemplatedLoopDetector:localizeRemote() loop use_nss, ns_factor : %lf\n", ns_factor);
                }

                if(!m_params.use_nss || ns_factor >= m_params.min_nss_factor)
                {
                    LOGI("TemplatedLoopDetector:localizeRemote() ns_factor >= m_params.min_nss_factor");
                    ...
                    if(!qret.empty()) {
                        LOGI("TemplatedLoopDetector:localizeRemote() after removeLowScores QueryResults size: %d", qret.size());
                        //printf("qret not empty\n");
                        // the best candidate is the one with highest score by now
                        match.match = qret[0].Id;
                        LOGI("TemplatedLoopDetector:localizeRemote() after removeLowScore match.match %d", int(match.match));
        ...
        /* No need to update because this is remote localization
        // update record
...
bool TemplatedLoopDetector<TDescriptor, F>::detectLoop(
  ...
  std::vector<cv::Point2f> &old_pts)
{
    LOGI("TemplatedLoopDetector:detectLoop()");
  //LOGI("TemplatedLoopDetector:detectLoop() m_params.k %d m_params.alpha %.2f"
    //      ,m_params.k, m_params.alpha);
  EntryId entry_id = m_database->size();
    LOGI("TemplatedLoopDetector:detectLoop() entry_id %d", int(entry_id));
  match.query = entry_id;
  ...
  if(m_params.geom_check == GEOM_DI) {
    ...
    m_database->getVocabulary()->transform(descriptors, bowvec);
  LOGI("TemplatedLoopDetector:detectLoop() entry_id %d m_params.dislocal %d",
          int(entry_id), int(m_params.dislocal));
    if((int)entry_id <= m_params.dislocal)
  {
    // only add the entry to the database and finish
