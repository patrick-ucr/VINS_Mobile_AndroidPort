...
public:
   ...
   */
    ...
    bool runRemote(const std::string &name,
             const std::vector<cv::KeyPoint> &keys,
             const std::vector<TDescriptor> &descriptors,
             std::vector<cv::Point2f> &cur_pts,
             std::vector<cv::Point2f> &old_pts,
    ...
    params(height, width), voc(vocfile), detector(voc, params)
{
    ...
    LOGI("loop image size width: %d height: %d\n", params.image_cols,params.image_rows);
}
...
// ---------------------------------------------------------------------------
template<class TVocabulary, class TDetector, class TDescriptor>
bool demoDetector<TVocabulary, TDetector, TDescriptor>::runRemote
        (const std::string &name, const std::vector<cv::KeyPoint> &keys,
         ...
         int &old_index)
{
    LOGI("demoDetector::runRemote()");
    int count = 0;

    DetectionResult result;

    detector.localizeRemote(keys, descriptors, result, cur_pts, old_pts);

    if(result.detection())
    {
        cout << "- loop found with image " << result.match << "!"
             << endl;
        LOGI("demoDetector::runRemote() localize success with image %d",int(result.match));
        ++count;
        cout << endl;
 ...
 int &old_index)
{
  ...
  if(result.detection())
  {
      cout << "- loop found with image " << result.match << "!"
        << endl;
      LOGI("demoDetector::run() loop found with image %d",int(result.match));
      ++count;
      cout << endl;
      old_index = result.match;
      return true;
  ...
  else
  {
      ...
      switch(result.status)
      {
        case CLOSE_MATCHES_ONLY:
          cout << "All the images in the database are very recent" << endl;
          LOGI("demoDetector::run() CLOSE_MATCHES_ONLY");
          break;
          
        case NO_DB_RESULTS:
          cout << "There are no matches against the database (few features in"
            " the image?)" << endl;
          LOGI("demoDetector::run() NO_DB_RESULTS");
          break;
          
        case LOW_NSS_FACTOR:
          cout << "Little overlap between this image and the previous one"
            << endl;
          LOGI("demoDetector::run() LOW_NSS_FACTOR");
          break;
            
        case LOW_SCORES:
          cout << "No match reaches the score threshold (alpha: " <<
            params.alpha << ")" << endl;
          LOGI("demoDetector::run() LOW_SCORES");
          break;
          
        case NO_GROUPS:
          cout << "Not enough close matches to create groups. "
            << "Best candidate: " << result.match << endl;
          LOGI("demoDetector::run() NO_GROUPS");
          break;
          
        case NO_TEMPORAL_CONSISTENCY:
          cout << "No temporal consistency (k: " << params.k << "). "
            << "Best candidate: " << result.match << endl;
          LOGI("demoDetector::run() NO_TEMPORAL_CONSISTENCY");
          break;
          
        case NO_GEOMETRICAL_CONSISTENCY:
          cout << "No geometrical consistency. Best candidate: " 
            << result.match << endl;
          LOGI("demoDetector::run() NO_GEOMETRICAL_CONSISTENCY");
          break;
          
        default:
            LOGI("demoDetector::run() default");
            break;
      }
      cout << endl;
      return false;
