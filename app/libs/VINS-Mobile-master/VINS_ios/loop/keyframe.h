...
public:
    ...
    void vec3dToPt3f(std::vector<cv::Point3f> & pc, std::vector<Eigen::Vector3d> point_clouds);

    void int2Double(std::vector<double> & nf_id, std::vector<int> f_id);

    void searchByDes_xukan(std::vector<cv::Point2f> &measurements_old,
    ...
    void PnPRANSAC_Remote(const vector<cv::Point2f> &matched_2d_old_norm,
