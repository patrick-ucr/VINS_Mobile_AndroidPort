...
bool LOOP_CLOSURE = true;//true Patrick
...
void VINS::setExtrinsic()
{
    ...
    TIC_Z;
    ric = Utility::ypr2R(Vector3d(RIC_y,RIC_p,RIC_r));
    printf("vins set extrinsic %lf %lf %lf\n", tic.x(), tic.y(), tic.z());
}
void VINS::old2new()
{
...
void VINS::update_loop_correction()
{
    ...
    // Patrick
    /*printf("correct_point_cloud.size()=%d",correct_point_cloud.size());
    int num_print = 0;
    for (auto point : correct_point_cloud){
        if (num_print++ > 10) break;
