package com.thkoeln.jmoeller.vins_mobile_androidport.tracking;
...
import com.thkoeln.jmoeller.vins_mobile_androidport.Classifier.Recognition;
import com.thkoeln.jmoeller.vins_mobile_androidport.env.BorderedText;
...
public class MultiBoxTracker {
    ...
    public List<RectF> getTrackedBoundingBoxes(){
        if (trackedObjects == null){
            return new ArrayList<>();
        }
        ArrayList<RectF> listObj = new ArrayList<>();
        final LinkedList<TrackedRecognition> copyList =
                new LinkedList<TrackedRecognition>(trackedObjects);
        for (final TrackedRecognition recognition : copyList) {
            ObjectTracker.TrackedObject trackedObj = recognition.trackedObject;
            if (trackedObj == null)
                continue;
            RectF location = trackedObj.getTrackedPositionInPreviewFrame();
            logger.i("Recognization location: "+location);
            listObj.add(new RectF(location));
        }
        logger.i("listObj size(): "+listObj.size());
        for (RectF objLoc : listObj){
            logger.i("Obj location: " + objLoc);
        }
        return listObj;
    }
