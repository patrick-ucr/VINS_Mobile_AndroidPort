...
Vector3f ViewController::getLatestTranslation(){
    return lateast_P;
}

Matrix3f ViewController::getLatestRotation(){
    return lateast_R;
}

vector<float> ViewController::projectPointCloudToScreenFloatVector(vector<float> pc_vfloat, Vector3f P, Matrix3f R) {
    vector<Vector3f> point_cloud;
    for (int i = 0; i < pc_vfloat.size(); i = i + 3){
        Vector3f pc(pc_vfloat[i],pc_vfloat[i+1],pc_vfloat[i+2]);
        point_cloud.push_back(pc);
    }
    vector<Point2f> screenPts = projectPointCloudToScreen(point_cloud,P,R);
    vector<float> results;
    for (auto pts: screenPts){
        results.push_back(pts.x);
        results.push_back(pts.y);
    }
    return results;
}

vector<cv::Point2f> ViewController::projectPointCloudToScreen(vector<Vector3f> point_cloud, Vector3f P, Matrix3f R){
    Eigen::Matrix3f RIC;
    ...
    {
        Vector3f Pc;
        Vector3f pc_3d = point_cloud[i];
        Pc = (R * RIC).transpose() * (pc_3d - 1.0 * P  - R * Vector3f(TIC_X,TIC_Y,TIC_Z));

        cv::Point2f pts;
        pts.x = FOCUS_LENGTH_X * Pc.x() / Pc.z()+ PX;
        pts.y = FOCUS_LENGTH_Y * Pc.y() / Pc.z()+ PY;

        LOGI("Projection 3D x,y,z: %.2f, %.2f, %.2f",pc_3d.x(),pc_3d.y(),pc_3d.z());
        LOGI("Projection 2D x,y: %.2f, %.2f",pts.x,pts.y);
...
vector<float> ViewController::getObjectPointCloud(int left, int top, int right, int bottom){
    // VINS initialized
    vector<float> results;
    vector<Vector3f> results_vec3f;
    if (vins.solver_flag == vins.NON_LINEAR && keyframe_database.size() > 0) {
        KeyFrame *cur_kf = keyframe_database.getLastKeyframe();
        std::vector<cv::Point2f> measurements = cur_kf->measurements;
        std::vector<Eigen::Vector3d> point_clouds = cur_kf->point_clouds;
        LOGI("Size of point_clouds/measurements: %d %d",
             point_clouds.size(),
             measurements.size()
        );

        // index of measurements array that lie in bbox
        std::vector<int> idx;
        for(int i = 0; i < (int)measurements.size(); i++){
            cv::Point2f meas = measurements[i];
            int meas_x = (int) meas.x;
            int meas_y = (int) meas.y;
            // 2d feature lies in the bounding box
            if (meas_x >= left && meas_x <= right
                && meas_y >= top && meas_y <= bottom){
                idx.push_back(i);
            }
        }
        int out_pc_size = (int) idx.size();
        LOGI("out_pc_size %d",out_pc_size);
        for (auto i: idx){
            Eigen::Vector3d pts = point_clouds[i];
            Vector3f pts_vec3f ;
            pts_vec3f << (float) pts.x(), (float) pts.y(), (float) pts.z();
            results_vec3f.push_back(pts_vec3f);
            results.push_back((float) pts.x());
            results.push_back((float) pts.y());
            results.push_back((float) pts.z());
        }

        //vector<cv::Point2f> screenPixels = projectPointCloudToScreen(results_vec3f, lateast_P,lateast_R);
        // Emulation Patrick add constant translation (0,0,+1) of another phone
        //Vector3f b_translation(1.0,0.0,0.0);
        //Vector3f new_P = lateast_P + b_translation;
        //Matrix3f old_R = lateast_R;
        //LOGI("After translation of B");
        //vector<cv::Point2f> screenPixels_B = projectPointCloudToScreen(results_vec3f, new_P, old_R);
    }

    return results;
}

float ViewController::estimateObjectDistanceInBox(int left, int top, int right, int bottom){
    // VINS initialized
    if (vins.solver_flag == vins.NON_LINEAR && keyframe_database.size() > 0) {
        KeyFrame *cur_kf = keyframe_database.getLastKeyframe();
        std::vector<cv::Point2f> measurements = cur_kf->measurements;
        ...
        float pc_size = point_clouds.size();
        //float sum_x = 0.0, sum_y = 0.0, sum_z = 0.0;
        for(int i = 0; i < (int)point_clouds.size(); i++) {
            Eigen::Vector3d pcld = point_clouds[i];
            cv::Point2f meas = measurements[i];
            LOGI("Point cloud x,y,z: %.2f, %.2f, %.2f",pcld.x(),pcld.y(),pcld.z());
            LOGI("Measurement feature x,y: %.2f, %.2f",meas.x,meas.y);
            //sum_x += pts.x();
            //sum_y += pts.y();
            //sum_z += pts.z();
        }
        //LOGI("Average x,y,z: %.2f, %.2f, %.2f",sum_x/pc_size,sum_y/pc_size,sum_z/pc_size);
        std::vector<int> idx;
        for(int i = 0; i < (int)measurements.size(); i++){
            cv::Point2f meas = measurements[i];
            int meas_x = (int) meas.x;
        ...
        LOGI("idx.size(): %d",idx.size());
        float dist = 0.0;
        for (auto i: idx){
            Eigen::Vector3d pts = point_clouds[i];
            dist += (float) pts.y();
            LOGI("point cloud location: %.2f, %.2f, %.2f",pts.x(),pts.y(),pts.z());
        }
        return dist/(float) idx.size(); // average all pointcloud distances
    } else {
        return -1.0;
    }
...
//        TE(time_feature);
        ...
        LOGI("viewController: processImage: good_pts.size() %d track_len %d", good_pts.size(),track_len.size());
        ...
        if(imageCacheEnabled)
        {
            ...
            if(!vins_pool.empty() && !image_pool.empty())
            {
                ...
                if(!vins_pool.empty() && !image_pool.empty())
                {
                    ...
                    LOGI("latest P,R from vins_pool");
		...
		// Patrick
		LOGI("lateast_P: %.2f,%.2f,%.2f", lateast_P(0),lateast_P(1),lateast_P(2));
        LOGI("lateast_R: %.2f,%.2f,%.2f\n%.2f,%.2f,%.2f\n%.2f,%.2f,%.2f", lateast_R(0,0),lateast_R(0,1),lateast_R(0,2),lateast_R(1,0),lateast_R(1,1),lateast_R(1,2),lateast_R(2,0),lateast_R(2,1),lateast_R(2,2));
...
//                        Vector3d pts_i = it_per_id.estimated_depth * it_per_id.feature_per_frame[0].point;
        ...
        if(LOOP_CLOSURE)
        {
            LOGI("LOOP_CLOSURE enabled");
            LOGI("keyframe_freq %d",keyframe_freq);
...
// copy implementation from keyframe.cpp
template<typename T>
...
// Using keyframe_database to get translation and rotation from a remote image
void ViewController::loopDetectionRemote(cv::Mat &image) {
    LOGI("ViewController::loopDetectionRemote");
    if (vins.solver_flag != vins.NON_LINEAR || keyframe_database.size() <= 0){
        return;
    }
    ...
    if (loop_closure == NULL)
        return;
    LOGI("loopDetectionRemote started");
    LOGI("loopDetectionRemote keyframe_database.size(): %d",keyframe_database.size());
    const char *pattern_file = "/storage/emulated/0/VINS/brief_pattern.yml"; // [[[NSBundle bundleForClass:[self class]] pathForResource:@"brief_pattern" ofType:@"yml"] cStringUsingEncoding:[NSString defaultCStringEncoding]];
    Vector3d T_w_i = Vector3d::Identity() ;//= vins.Ps[WINDOW_SIZE - 2];
    Matrix3d R_w_i = Matrix3d::Identity() ;//= vins.Rs[WINDOW_SIZE - 2];
    // dummy keyframe constructed from remote host image
    KeyFrame* remote_kf = new KeyFrame(0.0, 0, T_w_i, R_w_i, image, pattern_file, 0);
    LOGI("loopDetectionRemote Keyframe constructed");
    //keyframe->setExtrinsic(vins.tic, vins.ric);
    /*
     ** we still need save the measurement to the keyframe(not database) for add connection with looped old pose
     ** and save the pointcloud to the keyframe for reprojection search correspondance
    ...
    current_image = remote_kf->image;
    remote_kf->extractBrief(current_image);
    remote_kf->setExtrinsic(vins.tic, vins.ric);
    LOGI("loopDetectionRemote BRIEF extracted");
    LOGI("loopDetectionRemote keypoints size: %d",remote_kf->keypoints.size());
    LOGI("loopDetectionRemote descriptor size: %d",remote_kf->descriptors.size());
    LOGI("loopDetectionRemote measurements size: %d",remote_kf->measurements.size());
    vector<cv::Point2f> cur_pts;
    vector<cv::Point2f> old_pts;
    std::vector<cv::Point2f> measurements_old;
    std::vector<cv::Point2f> measurements_old_norm;
    vector<cv::Point3f> matched_3d;//ok
    vector<double> matched_id;//ok
    vector<uchar> status;

    int old_index_now = -1;
    LOGI("loopDetectionRemote before startLoopClosure");
    bool loop_succ = false;

    TS(Loop_Detection_Remote);
    // Patrick
    //if (!detecting_loop) {
    //    detecting_loop = true;
        loop_succ = loop_closure->startRemoteLocalize(
                remote_kf->keypoints, remote_kf->descriptors,
                cur_pts, old_pts,
                old_index_now);
        LOGI("loopDetectionRemote cur_pts %d old_pts %d",cur_pts.size(),old_pts.size());
    //}
    //detecting_loop = false;
    TE(Loop_Detection_Remote);

    vector<cv::Point2f> matched_2d_cur, matched_2d_old;//matched_2d_cur: measurements
    vector<cv::Point2f> matched_2d_cur_norm, matched_2d_old_norm;//matched_2d_cur_norm OK,
    if (!loop_succ){
        LOGI("loopDetectionRemote startLoopClosure found loop FAIL");
    } else {
        LOGI("loopDetectionRemote startLoopClosure found loop SUCCESS with index %d",old_index_now);
        KeyFrame *old_kf = keyframe_database.getKeyframe(old_index_now);
        if (old_kf == NULL) {
            printf("loopDetectionRemote NO such %dth frame in keyframe_database\n", old_index_now);
            assert(false);
        }
        LOGI("loopDetectionRemote loop succeeded with %d image\n", old_index_now);
        assert(old_index_now != -1);
        ...
        LOGI("loopDetectionRemote old_kf translation %.2lf %.2lf %.2lf",T_w_i_old.x(),T_w_i_old.y(),T_w_i_old.z());
        LOGI("loopDetectionRemote old_kf rotation\n%.2lf %.2lf %.2lf\n%.2lf %.2lf %.2lf\n%.2lf %.2lf %.2lf",
             R_w_i_old(0,0),R_w_i_old(0,1),R_w_i_old(0,2),
             R_w_i_old(1,0),R_w_i_old(1,1),R_w_i_old(1,2),
             R_w_i_old(2,0),R_w_i_old(2,1),R_w_i_old(2,2)
             );
        // not work returning identity matrices
        /*keyframe->findConnectionWithOldFrame(
                old_kf,cur_pts, old_pts,
                measurements_old, measurements_old_norm);

        Vector3d T_w_i_new;
        Matrix3d R_w_i_new;
        keyframe->getPose(T_w_i_new,R_w_i_new);
        LOGI("loopDetectionRemote keyframe translation %.2lf %.2lf %.2lf",T_w_i_new.x(),T_w_i_new.y(),T_w_i_new.z());
        LOGI("loopDetectionRemote keyframe rotation\n%.2lf %.2lf %.2lf\n%.2lf %.2lf %.2lf\n%.2lf %.2lf %.2lf",
             R_w_i_new(0,0),R_w_i_new(0,1),R_w_i_new(0,2),
             R_w_i_new(1,0),R_w_i_new(1,1),R_w_i_new(1,2),
             R_w_i_new(2,0),R_w_i_new(2,1),R_w_i_new(2,2)
        );*/
        // adapt from Xukan's findConnection -- swap cur_kf, old_kf
        vector<uchar> status1, status2;
        LOGI("loopDetectionRemote before searchByDes_xukan");
        old_kf->searchByDes_xukan(matched_2d_old, matched_2d_old_norm, remote_kf->descriptors, remote_kf->keypoints, status1, status2);
        LOGI("loopDetectionRemote after searchByDes_xukan matched_2d %d %d %d %d",
                matched_2d_old.size(),matched_2d_old_norm.size(),status1.size(),status2.size());

        old_kf->vec3dToPt3f(matched_3d, old_kf->point_clouds);
        old_kf->int2Double(matched_id,old_kf->features_id);
        LOGI("loopDetectionRemote matched_3d: %d matched_id %d",
             matched_3d.size(), matched_id.size());

        matched_2d_cur = old_kf->measurements;
        matched_2d_cur_norm = old_kf->pts_normalize;//not used??
        LOGI("loopDetectionRemote matched_2d_cur: %d matched_2d_cur_norm %d",
                matched_2d_cur.size(), matched_2d_cur_norm.size());
        LOGI("loopDetectionRemote 04.1, %d, %d, %d, %d, %d ", matched_id.size(), matched_2d_old_norm.size(), matched_3d.size(), matched_2d_cur_norm.size(), status1.size());
        // Patrick prevent null pointer dereference on reduceVector2
        // searchByDes_xukan fail to find matching even though loopDetection was success
        if (status1.size() == 0 || status2.size() == 0) {
            LOGI("loopDetectionRemote return before reduceVector2");
            return;
        }
        reduceVector2(matched_id, status1);
        reduceVector2(matched_2d_cur, status1);
        reduceVector2(matched_3d, status1);
        reduceVector2(matched_2d_cur_norm, status1);

        //but need status2 for matched_2d_old and matched_2d_old_nor
        reduceVector2(matched_2d_old, status2);
        reduceVector2(matched_2d_old_norm, status2);
        reduceVector2(matched_id, status2);
        reduceVector2(matched_2d_cur, status2);
        reduceVector2(matched_3d, status2);
        reduceVector2(matched_2d_cur_norm, status2);

        LOGI("loopDetectionRemote 04.2, %d, %d, %d, %d, %d ", matched_id.size(), matched_2d_old_norm.size(), matched_3d.size(), matched_2d_cur_norm.size(), status2.size());

        Eigen::Vector3d PnP_T_old;
        Eigen::Matrix3d PnP_R_old;
        LOGI("loopDetectionRemote before PnPRANSAC, size:%d ", matched_2d_cur.size());
        Eigen::Vector3d origin_T_w_i_old_kf;
        Eigen::Matrix3d origin_R_w_i_old_kf;
        old_kf->getOriginPose(origin_T_w_i_old_kf, origin_R_w_i_old_kf);
        remote_kf->updateOriginPose(origin_T_w_i_old_kf,origin_R_w_i_old_kf);

        if ((int)matched_2d_cur.size() > MIN_LOOP_NUM_RELOCATE)
        {
            LOGE("testoftransform 05");
            status.clear();
            //if(xukan_debug_findcon) printf("good at findcon 4\n");
            //0930
            //PnPRANSAC(matched_2d_old_norm, matched_3d, status, PnP_T_old, PnP_R_old);
            //cur3d,
            //old_kf->PnPRANSAC(matched_2d_old_norm, matched_3d, status, PnP_T_old, PnP_R_old);
            //remote_kf->PnPRANSAC(matched_2d_old_norm, matched_3d, status, PnP_T_old, PnP_R_old);
            remote_kf->PnPRANSAC_Remote(matched_2d_old_norm, matched_3d, T_w_i_old, R_w_i_old, status, PnP_T_old, PnP_R_old);
            //if(xukan_debug_findcon) printf("good at findcon 3, status size:%d\n", status.size());
            reduceVector2(matched_2d_cur, status);
            reduceVector2(matched_2d_old, status);
            //reduceVector2(matched_2d_cur_norm, status);
            reduceVector2(matched_2d_old_norm, status);
            reduceVector2(matched_3d, status);
            reduceVector2(matched_id, status);
            LOGI("loopDetectionRemote after PnPRANSAC, size:%d ", matched_2d_cur.size());
            LOGI("loopDetectionRemote old_kf translation after connection %.2lf %.2lf %.2lf",PnP_T_old.x(),PnP_T_old.y(),PnP_T_old.z());
            LOGI("loopDetectionRemote old_kf rotation after connection\n%.2lf %.2lf %.2lf\n%.2lf %.2lf %.2lf\n%.2lf %.2lf %.2lf",
                 PnP_R_old(0,0),PnP_R_old(0,1),PnP_R_old(0,2),
                 PnP_R_old(1,0),PnP_R_old(1,1),PnP_R_old(1,2),
                 PnP_R_old(2,0),PnP_R_old(2,1),PnP_R_old(2,2)
            );

        }
    }
    remote_kf->image.release();
    delete remote_kf;
}

void ViewController::loopDetectionLoop() {
//-(void)loop_thread{
    LOGI("loopDetectionLoop keyframe_database.size(): %d",keyframe_database.size());
...
//        LOGI("THREAD: loop_thread(loopDetectionLoop)");
        ...
        if (loop_check_cnt < global_frame_cnt) {
            ...
            if(loop_succ) {
                ...
                LOGI("loop succeeded with %drd image\n", old_index);
                assert(old_index!=-1);
...
//{
    ...
    return DeviceType::GooglePixel2;
    //return DeviceType::LGG6;
	/*
