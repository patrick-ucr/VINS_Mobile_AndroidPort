...
// MotionManager for read imu data
    ...
    const int SENSOR_REFRESH_RATE_HZ = 100;//100
...
// Indicate the loop frame index
    ...
    // Patrick lock loop detection
    bool detecting_loop = false;
// Translation drift
 ...
 */
    ...
    vector<float> getObjectPointCloud(int left, int top, int right, int bottom);

    float estimateObjectDistanceInBox(int left, int top, int right, int bottom);

    vector<cv::Point2f> projectPointCloudToScreen(vector<Vector3f> point_cloud, Vector3f P, Matrix3f R);

    vector<float> projectPointCloudToScreenFloatVector(vector<float> pc_vfloat, Vector3f P, Matrix3f R);

    Vector3f getLatestTranslation();
 ...
 */
    ...
    // Patrick: use loop detection as a localization of a remote phone using remote image/frame
    void loopDetectionRemote(cv::Mat& image);
    //TODO: solve quick fix
