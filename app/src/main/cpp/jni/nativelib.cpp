...
Java_com_thkoeln_jmoeller_vins_1mobile_1androidport_VinsJNI_getObjectPointCloud(JNIEnv *env, jclass type,
                                                                              jint left,
                                                                              jint top,
                                                                              jint right,
                                                                              jint bottom) {
    ...
    int r = right;
    int b = bottom;
    vector<float> objectPointCloud = viewControllerGlobal->getObjectPointCloud(l,t,r,b);
    int len = objectPointCloud.size();
    LOGI("objectPointCloud len %d",len);
    // remove first element holding length of array
    //float pc[len];
    /*for (int i = 0; i < len; i++){
        pc[i] = objectPointCloud[i+1];
        LOGI("pc[%d] = %.2f",i,pc[i]);
    }*/
    //std::copy(objectPointCloud+1,objectPointCloud+len,pc);
    jfloatArray result = (jfloatArray) env->NewFloatArray(len);
    env->SetFloatArrayRegion(result,0,len,&objectPointCloud[0]);
    return result;
}

extern "C"
JNIEXPORT jfloatArray JNICALL
Java_com_thkoeln_jmoeller_vins_1mobile_1androidport_VinsJNI_projectPointCloudToScreen(JNIEnv *env, jclass type,
                                                                                jint size,
                                                                                jfloatArray point_cloud) {
    vector<float> pc;
    jsize len = env->GetArrayLength(point_cloud);
    jfloat *body = env->GetFloatArrayElements( point_cloud, 0);
    LOGI("projectPointCloudToScreen %d %d",size,len);
    for (int i; i<size; i++){
        pc.push_back((float) body[i]);
    }
    Vector3f p = viewControllerGlobal->getLatestTranslation();
    Matrix3f r = viewControllerGlobal->getLatestRotation();
    Vector3f b_translation(1.0,0.0,0.0);
    Vector3f new_P = p + b_translation;
    Matrix3f old_R = r;
    vector<float> screenPts = viewControllerGlobal->projectPointCloudToScreenFloatVector(pc,new_P,old_R);
    int sp_len = screenPts.size();
    jfloatArray result = (jfloatArray) env->NewFloatArray(sp_len);
    env->SetFloatArrayRegion(result,0,sp_len,&screenPts[0]);
    return result;
}

extern "C"
JNIEXPORT jfloat JNICALL
Java_com_thkoeln_jmoeller_vins_1mobile_1androidport_VinsJNI_getObjectDistance(JNIEnv *env, jclass type,
                                                                                jint left,
                                                                                jint top,
                                                                                jint right,
                                                                                jint bottom) {
    ...
    int r = right;
    int b = bottom;
    float dist = viewControllerGlobal->estimateObjectDistanceInBox(l,t,r,b);
    return dist;
}

extern "C"
JNIEXPORT void JNICALL
Java_com_thkoeln_jmoeller_vins_1mobile_1androidport_VinsJNI_onImageAvailableRemote(JNIEnv *env, jclass type,
...
//    LOGI("buf.stride: %d", buf.stride);
    ...
    viewControllerGlobal->loopDetectionRemote(rotatedRgba);
...
Java_com_thkoeln_jmoeller_vins_1mobile_1androidport_VinsJNI_updateViewInfo(JNIEnv *env, jclass type,
    ...
    if(viewControllerGlobal->tvXText.empty() == false) {
        LOGE("Xukan updateViewInfo NOT empty");
        ...
        LOGE("Xukan updateViewInfo IS empty");
    }
...
extern "C"
JNIEXPORT void JNICALL
Java_com_thkoeln_jmoeller_vins_1mobile_1androidport_VinsJNI_setARTouchPosition(JNIEnv *env, jclass type,
                                                                     jfloat x, jfloat y) {

    viewControllerGlobal->vins.drawresult.tapFlag = true;
    viewControllerGlobal->vins.drawresult.locationTapX = y;
    viewControllerGlobal->vins.drawresult.locationTapY = x;
}
